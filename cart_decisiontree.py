#-*-coding:utf8-*-
#user:brian
#created_at:2019/11/5 8:34
# file: cart_decisiontree.py
...
###定义一个函数输入子节点：也就是分类的结果
def get_leaf_Data(decisionTree):
    global dcHeadings
    dcNodes = defaultdict(list)
    """Plots the obtained decision tree. """
    ...
    for nSplit in range(len(dcNodes)):
        ...
        for lsX in lsY:
            ...
            if type(iSplit) != int:
                temp={}
                temp["sample"]=szSamples
                decision_Data=re.split(":|,",decision) #根据样式进行分割，得到最后的结果包括状态，以及样本数量和比例等。
                state_dict={}
                for i in range(int(len(decision_Data)/2)):
                        i=i*2
                        state_dict[decision_Data[i]]=decision_Data[i+1]
